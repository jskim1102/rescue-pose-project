...
const POSTURE_LABEL_COLOR: Record<string, string> = {
  lying: "#e5484d",
  sitting: "#e0a23b",
  standing: "#3fb950",
};
...
function KeypointOverlay({
  ...
  useEffect(() => {
    ...
    for (const person of detections) {
      ...
      // rescue-pose 확장: rtsp-keypoint 스켈레톤 렌더 뒤에 자세 라벨만 덧그린다.
      if (person.posture) {
        const visible = keypoints.filter((point) => point[2] >= KPT_CONF_THRESHOLD);
        if (visible.length > 0) {
          const minX = Math.min(...visible.map((point) => point[0] * sx));
          const minY = Math.min(...visible.map((point) => point[1] * sy));
          const text = person.posture.toUpperCase();
          const padX = 5;
          const padY = 3;
          ...
          const top = Math.max(0, minY - height - 2);
          ctx.fillStyle = POSTURE_LABEL_COLOR[person.posture] ?? "#8b95a5";
          ctx.fillRect(minX, top, width, height);
          ctx.fillStyle = "#0a0d12";
          ctx.fillText(text, minX + padX, top + padY);
        }
      }
    }
