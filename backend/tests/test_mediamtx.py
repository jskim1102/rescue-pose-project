...
def test_register_rejects_shell_metachars(evil):
    ...
    `&` 는 예외 — RTSP query string 의 정상 문자라 허용한다(아래 positive 케이스 참조).
    """
    with patch.object(mediamtx, "MEDIAMTX_API", "http://mtx:9997"), \
         patch("app.mediamtx.httpx.post") as post:
        with pytest.raises(ValueError, match="금지문자"):
...
def test_stream_rtsp_url_uses_backend_auth_and_base():
    """Detection 은 원본 카메라가 아니라 mediamtx RTSP fan-out 을 backend 인증으로 읽는다."""
    with patch.object(mediamtx, "MEDIAMTX_RTSP_BASE_URL", "rtsp://mtx.local:8554"), \
         patch.object(mediamtx, "MEDIAMTX_BACKEND_USER", "backend"), \
         patch.object(mediamtx, "MEDIAMTX_BACKEND_PASS", "s3cret"):
        assert (
            mediamtx.stream_rtsp_url("rescue_pose__ipcam-f1937574")
            == "rtsp://backend:s3cret@mtx.local:8554/rescue_pose__ipcam-f1937574"
        )
