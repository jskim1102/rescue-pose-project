...
@pytest.mark.parametrize(
    ...
    [
        (0.35, 1.0, 0.95, 0.35),  # 입력이 100%여도 제품 상한 95%로 clamp
...
def test_inference_api_clamps_gpu_target_to_95_percent(monkeypatch):
    manager = StreamManager()
    monkeypatch.setattr(inference_api, "stream_manager", manager)

    response = inference_api.update_inference_config(
        inference_api.InferenceConfigUpdate(gpu_util_target=1.0)
    )

    assert response["gpu_util_target"] == pytest.approx(0.95)


def test_manager_defaults_gpu_target_to_95_percent():
    manager = StreamManager()

    assert manager.get_inference_config()["gpu_util_target"] == pytest.approx(0.95)
