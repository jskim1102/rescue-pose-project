...
GPU_UTIL_TARGET_DEFAULT = 0.95
GPU_UTIL_TARGET_MIN = 0.1
GPU_UTIL_TARGET_MAX = 0.95
...
def _gpu_util_scale_step(
    ...
    *,
    ...
    """
    target = max(GPU_UTIL_TARGET_MIN, min(GPU_UTIL_TARGET_MAX, float(target)))
...
class StreamManager:
    ...
    def __init__(
        ...
        *,
        ...
        # rtsp-keypoint 결과에 rescue-pose의 posture 지속시간 판정만 후처리로 연결한다.
    ...
    def replace_source(self, source_id: str, new_source: SourceType) -> bool:
        ...
        **URL 권위로 기록**한다(idle ref-0 엔트리 신설). 변경했으면 True.
        ...
        if old is not None:
            ...
            self._clear_rescue_state(source_id)
                ...
                with self._lock:
                    if result.source_id in self._captures:
                        ...
                        if current is None or (
                            ...
                            # 구조 상태를 먼저 갱신한 뒤 결과를 공개한다. 반대 순서면 WS가
                            # 같은 timestamp를 rescue 상태 없이 한 번 보내고 재전송하지 않는
                            # 짧은 race가 생길 수 있다.
                            self._update_rescue(result)
                            self._latest_results[result.source_id] = result

    def _clear_rescue_state(self, source_id: str | None = None) -> None:
        # 일부 manager 단위 테스트는 rtsp-keypoint 정본처럼 __init__ 없이 최소 상태만
        # 조립한다. rescue seam이 없는 그 경로에서는 원본 lifecycle을 그대로 통과시킨다.
    ...
    def set_gpu_util_target(self, target: float) -> None:
        """전역 GPU duty ceiling 설정(10~95%)."""
        clamped = max(GPU_UTIL_TARGET_MIN, min(GPU_UTIL_TARGET_MAX, float(target)))
        with self._telemetry_lock:
...
def detections_to_json(result: InferenceResult) -> str:
    ...
    for index, detection in enumerate(result.detections):
        ...
        if detection.posture:
            item["posture"] = detection.posture
        rescue = (
            rescue_states[index]
            if rescue_states is not None and index < len(rescue_states)
            else None
        )
        if rescue is not None:
            if rescue.rescue_needed:
                item["rescueNeeded"] = True
            if rescue.lying_sec > 0:
                item["lyingSec"] = round(rescue.lying_sec, 1)
        items.append(item)
    return json.dumps({
        "type": "detections",
