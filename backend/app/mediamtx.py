...
def stream_rtsp_url(stream_key: str) -> str:
    """backend detection 이 읽을 mediamtx RTSP fan-out URL.

    WHEP 영상과 detection 이 서로 다른 원본 RTSP client 로 붙으면 카메라/중계기별 버퍼가 달라져
    좌표가 영상보다 늦을 수 있다. backend 도 mediamtx path 를 읽게 해 영상 fan-out 과 같은 시간축에
    최대한 붙인다.
    """
    parsed = urlsplit(MEDIAMTX_RTSP_BASE_URL.rstrip("/"))
    if parsed.scheme not in {"rtsp", "rtsps"} or not parsed.netloc:
        raise RuntimeError(f"MEDIAMTX_RTSP_BASE_URL 이 올바른 RTSP base URL 이 아닙니다: {MEDIAMTX_RTSP_BASE_URL!r}")

    netloc = parsed.netloc
    if "@" not in netloc and MEDIAMTX_BACKEND_PASS:
        host = parsed.hostname or ""
        if ":" in host and not host.startswith("["):
            host = f"[{host}]"
        port = f":{parsed.port}" if parsed.port else ""
        user = quote(MEDIAMTX_BACKEND_USER, safe="")
        password = quote(MEDIAMTX_BACKEND_PASS, safe="")
        netloc = f"{user}:{password}@{host}{port}"

    base_path = parsed.path.rstrip("/")
    path = f"{base_path}/{quote(stream_key.strip('/'), safe='')}" if base_path else f"/{quote(stream_key.strip('/'), safe='')}"
